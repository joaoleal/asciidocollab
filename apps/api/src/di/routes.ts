...
import { gitConflictsRoutes } from '../routes/projects/git/conflicts';
import { gitPullCompleteRoutes } from '../routes/projects/git/pull-complete';
import { gitUndoPullRoutes } from '../routes/projects/git/undo-pull';
...
export async function registerRoutes(app: FastifyInstance): Promise<void> {
  ...
  await app.register(async function protectedRoutes(scopedApp: FastifyInstance) {
    ...
    await scopedApp.register(async function verifiedRoutes(innerApp: FastifyInstance) {
      ...
      await innerApp.register(gitConflictsRoutes);
      await innerApp.register(gitPullCompleteRoutes);
      await innerApp.register(gitUndoPullRoutes);
