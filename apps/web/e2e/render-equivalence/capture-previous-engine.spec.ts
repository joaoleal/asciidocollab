...
test.describe('previous-engine capture', () => {
  ...
  test('renders every corpus document through the app render path and captures it', () => {
    ...
    for (const [index, document] of documents.entries()) {
      ...
      const html = renderCorpusDocument(document, index + 1);
  ...
  test('carries the provenance and placeholders the app itself emits', () => {
    const documents = corpusDocuments();

    const withDiagrams = documents.find((document) => document.name === 'diagrams-stem');
    expect(withDiagrams).toBeDefined();
    const diagramsHtml = readFixture(withDiagrams!.name) ?? renderCorpusDocument(withDiagrams!, 900);
    ...
    const includesHtml = readFixture(withIncludes!.name) ?? renderCorpusDocument(withIncludes!, 901);
