...
# This script brings the stack up and holds it up: it stays in the foreground and
# its EXIT trap runs `docker compose down -v`, so the stack lives exactly as long
# as this process does — which is why holding the lock for this process's lifetime
# is the right scope, not merely a formality that lapses when it returns.
# scripts/ci/e2e-local.sh would otherwise begin by tearing this stack down.
# Invariant and mechanism: scripts/lib/e2e-lock.sh.
e2e_lock_guard "e2e-stack-up" "$SELF" "$@"
...
e2e_assert_stack_not_in_use "e2e-stack-up"
...
# step.)
e2e_export_stack_owner "e2e-stack-up"
